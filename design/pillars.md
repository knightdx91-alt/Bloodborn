...
| L54 | Engine: **Godot 4** — *revised 2026-09-14, was Unity* | `tech.md` §2 and §2a. **The original lock chose Unity over Unreal 5** on two arguments: that Unity is all text and therefore legible to an AI collaborator, and that its asset marketplace could carry §1's buy-the-content strategy. Both were undermined by evidence the lock did not have. **Godot is text too, and more so** — it is also the only one of the three an assistant can build, run, export and *look at* end to end with no editor, no GPU and nothing on the developer's machine, which is how Stage 1 steps 1–3 came to exist at all. **The marketplace argument mostly dissolved:** Unity's own documentation permits Asset Store assets in other engines, so the store is not Unity-only; the residue is editor tooling, which this project buys least of because §1 hand-writes its systems. **The console path inverted:** W4 Consoles is $2,000/yr for all three platforms with no revenue share, against Unity Pro at $2,310 per seat per year, which Unity *requires* to ship on console at all. What is given up is real and is recorded in §2a: a thinner tooling ecosystem, one small vendor carrying all three console ports, and **C# not reaching the web export** (L88). Unchanged from the original lock: **no engine's built-in networking is MMO-scale** — the zone-server layer is code to be written, not a package to install |
...
| L76 | Minimal intervention; the world is not policed | `moderation.md` §0. **This is a world people live in, not a space that is policed.** Automated word detection does nearly all the work, the world's own consequences (L74) do the rest, and **human action is reserved for the narrow set of things that are actually illegal** — no community-standards enforcement, no tone policing, no arbitration of player disputes, no penalty for being unpleasant. Someone merely rude is handled by other players blocking them and by a world that stops dealing with them. **What is not optional:** every console requires a working report path and a means of actioning reports before certifying player-to-player voice (L53 puts consoles on the road), and services carrying user-to-user communication have legal duties. So the report button must exist; the choice is *what gets acted on*, and the answer is the legal floor and nothing above it. **Voice is off below the platform age threshold** — mostly inherited, since consoles already gate it through parental controls; PC needs an age declaration at account creation, and P11 §9.7's full-fidelity text parity means a voiceless player loses only the microphone. Explicitly rejected: karma systems and community reputation scores, which become weapons within a week and turn an adversarial world into a popularity contest — reputation here is what people *say about you*, never a number the game keeps |
| L77 | The Revelation Arc is an overture, not the game | `liveops.md`. A server whose Arc is complete is not an empty world. **Everything structural in Marrowmark is cyclical by construction**: war rights are held for a season and contested again (L25), every repair costs an item its life so gear leaves the world forever and crafters refill it forever (L59), Incarnate seats cycle because the Interior's entropy guarantees every holder eventually falls (L10/L30), the Interior is a frontier nobody returns from, and the Monument accumulates for as long as the world runs. What the Arc spends is **one thing only** — the surprise that magic exists — and L36 already said that secret breaks at internet speed while the mechanical climb is what's paced. A newcomer in year three loses the surprise and keeps the climb, which spells this world has, who holds the seats, the Interior, and the secret canon |
| L78 | New worlds on a cadence; rebirth is a step, not a loss | Extends L16/L37/P12. The appetite for the Myth epoch — fog, rumour, nobody knowing anything — is real and cannot be served by an old server, so **new worlds open on a cadence and there is always a young one**. This is also L16's launch-surge answer already on file: *more worlds, never bigger ones*. Rebirth (L37) makes moving cheap, and because **P12 marks are account-level they come with you** — so starting again is not a loss but a step toward six. That closes a loop the design did not obviously have: **the second-year problem and the Age of Gods are the same mechanism seen from two ends**, the reason to start again and the reward for having done so. Old worlds are retired on population, never on age |
| L79 | Live-ops turns dials; it never authors history | `liveops.md` §5. Four dials only: the **awakening valve** (`feasibility-review.md` §3.3's "single most important live-ops dial", to be built as a dial rather than a constant), **epoch arming conditions** (L41 — the operator tunes the arming, never the firing), **new world cadence**, and **coin faucets by epoch**. Absolute prohibitions: never fire an epoch by hand (L23 promises player-advanced history and every player who was there will know); never inject a world event overriding player history — no scripted invasions, no angry gods; never run a season, battle pass or rotating shop (L27 leaves no treadmill to attach); never rebalance in a way that voids crafted goods, because every item carries a maker's name (L68) and their reputation is attached to it; and never resurrect a spent secret — nostalgia is served by a new world, not by rewinding an old one |
| L80 | World first, briefly on screen second, absent third | `interface.md`. **Players look at the world, not at menus.** For any information: put it in the world (a blade with notches, a man breathing hard, a wagon riding low); failing that put it on screen *briefly*, appearing when it matters and leaving when it does not; failing that put it in the companion app (L83); failing that **do without it** — which is a legitimate answer here in a way it is not in most games. **Nothing is permanently on screen**: no persistent HUD, no minimap, no compass, no hotbar, no objective tracker, no damage numbers, no loot or skill-up notifications. The stamina bar (the hard case, since L55 makes it the whole combat economy and reading it from breathing would be unfair) **appears while it is moving and fades when you are full** — exact when it matters, absent otherwise. Your opponent gets no bars at all: you read them by posture, breathing and whether their vambrace is still on, which L64/L65 already require |
...
| L88 | `sim/` stays C# and authoritative; the prototype mirrors it in GDScript, **with a named exit** | Forced by L54. `tech.md` §2. **Godot's web export cannot run C#** — Godot 4 lost C# everywhere except Windows, macOS and Linux — and the web build is the only way the developer plays anything without a PC. Shipping is unaffected: PC, and console via W4 (C# in beta there), both run C#. So `sim/` remains the authority — it is what the zone servers will run and what the tests cover — and `prototype/rules/` mirrors **only what the prototype actually exercises**. **Tuning is never mirrored**: every number lives once in `shared/tuning/combat.json`, and `TuningFileTests` fails the build in both directions if the copies drift. Logic diverging is a bug someone notices; numbers diverging is a month of tuning against the wrong game. **The exit, named so it cannot happen by drift:** when the developer can routinely run native builds, the prototype moves to Godot's .NET build and `prototype/rules/` is deleted the same day. Until then the mirror is a cost paid on purpose, and **anything the prototype does not exercise is never mirrored at all** |
| L89 | The sun is the compass | **There is a day/night cycle, one clock, shared by everyone on a server.** It is server-authoritative like everything else — two players standing together see the same light, and "meet me at dusk" is a thing you can say and mean. **The sun rises east and sets west, everywhere, always**, which makes it the compass **L80** forbids on screen: L80's first preference is to put information *in the world*, and a sun overhead is the oldest instrument there is. With **L82**'s player-made maps it completes a navigation kit that needs no interface at all — a map tells you what somebody knew, the sun tells you which way you are pointing, and neither is a marker. It also makes **L28**'s "distance is real" felt rather than asserted: a journey that outruns the daylight is a decision about when to leave. **Night is genuinely dark and that is a real cost, not atmosphere.** `combat.md` §6 makes reading an attack off the body a hard requirement, and darkness attacks it directly — so night is when the world is most dangerous, carried light matters, and settlements are worth reaching before it falls. The design deliberately does **not** cheat this with bright moonlight or unexplained visibility (L20). **Open, and flagged rather than assumed:** how dark night actually gets before §6 breaks, what a carried light costs and occupies, and the day's real-time length — the principle is that a normal session should contain both day and night, with night a meaningful fraction and less than half. **Not a weather system**, which is a separate and much larger commitment |
| L90 | A menu is a place, not a screen | **The few menus the game does have are built to one specification, and there is one implementation of it.** **L80** keeps the world in front and allows an interface only where nothing else will do, which still leaves a conversation, the boards, the pack and character creation — and refusing to *design* those does not remove them, it only makes them bad. `interface.md` §8 sets the rules and `prototype/ui.gd` is the single place they live. **Nothing is sized in fixed pixels**: every measurement is a multiple of one scale taken from the viewport on *both* axes, because a menu tuned to a developer's window clips on a phone and a scale taken from height alone is the same bug wearing a hat. **Everything focusable is visibly focused, and something always is** — **L15** ships to three consoles, a pad cannot press what it cannot select, and a menu that opens with nothing selected reads as a broken controller rather than an unfocused one. **A presses, B backs out one layer**, neither of which Godot gives you. **A menu owns the sticks while it is up**, so no one walks out of a conversation they are holding. **A panel underneath stops taking focus**, or the d-pad walks the highlight behind the panel in front and A presses something invisible — which for **L49**'s confirm gate means signing by accident, and a gate you can step around is not a gate. **The gate therefore opens on the refusal**, never on the commitment. **Every choice is at least 48px tall in real pixels**, touch being a first-class input and not an afterthought |
| L91 | The game follows the hands, not the machine | **The input scheme is detected from the last input the player actually used, never declared in a menu.** **L15** ships to PC and three consoles — two schemes in one build already — and the prototype is played on a phone with a controller plugged into it, which belongs to neither platform. So the platform only chooses the *opening guess* (a pad if one is connected, else touch if the screen is one, else keyboard and mouse); after that every event is evidence, and the prompts and on-screen controls follow the hands. **Unplugging mid-session is a first-class case, not an error**: the controller leaves the socket and the thumb controls come back, with no menu, no restart and no dialogue. **The player is never asked to declare their hardware** — a settings screen listing input devices is exactly what `interface.md` §1 exists to refuse; the launcher *states* the detected scheme so that "where did the on-screen stick go" has a visible answer, but it is a readout, not a choice. **On-screen controls are drawn only for the scheme using them**, because a thumbstick painted over a controller game is the persistent HUD §1 forbids in a different hat. **Evidence has a threshold**: a stick drifting on a worn pad must not flip the interface back and forth while somebody types, so waking the pad takes a real push and a much larger dead zone than gameplay uses, and a mouse event arriving just after a touch is that touch's synthetic echo rather than a hand on a mouse. **Every scheme must be able to reach every part of the game** — the rule that catches the real bugs: "leave this place" was bound to Escape and to Start, and a phone without a controller has neither, so a touch player could walk into town and never walk out. **A scheme that cannot press anything is not a supported scheme, and nobody will notice on your behalf**: touch-to-mouse emulation was switched off project-wide from the drill yard's third commit to stop an emulated click firing a swing, and because Godot activates a Button from *mouse* events rather than raw touches, that single line meant no button anywhere in the game could be pressed with a finger — through months of play, unnoticed, because the pad did all the pressing. So a fix for one scene is never applied to the whole project when the scene can carry it, and every scheme is exercised deliberately on its own. **None of this replaces remapping** (§7): detection decides what is *shown*, remapping decides what things *do* |
| L92 | Topics answer to the world | **Every line an NPC offers is resolved against world state at the moment it is shown. A dialogue card is a CANDIDATE, not a script.** This is a standing rule over every NPC and everything they offer, not a special case for contracts. Reported from play: a contract taken off the board was still offered by the man who posted it, with the paper already in the player's hand — the same failure as a quest-giver repeating a finished quest, and it makes a town look like it is not listening. **The world contradicting an offer is not only completion**: an empty purse contradicts the offer of a drink, an apprenticeship already sworn contradicts "take me on", a caravan already gone contradicts escorting it. **Prefer CHANGING a topic to removing one — for the thing the player actually did**: "Escort the Vellmark wagon?" becoming "— taken", with somewhere to be and a time to be there, is a town that noticed, and reporting for duty is what the player wants at that moment. **What the world contradicts is the SAME offer, not the next one**: you may carry as much work as you can find, and a carter with two wagons to fill will give you both — what makes no sense is taking the same job twice. ("One piece of work at a time" was implemented and rejected in play: it hid every other offer, including apprenticeships, from anyone holding a single contract.) **CREATE a topic the world needs said and no card authored**: work finished out in the world has to be handed in to somebody who never had a line about it, because a contract is taken off a board rather than from a person — and who can settle it is declared by CONTENT (a capability on the roster card), never known by the conversation code, so a second town has its own clerk. **WITHHOLD a topic entirely — return nothing rather than grey it — only when the offer is IMPOSSIBLE**, such as escorting a caravan that has already departed; an "— unavailable" row is the game admitting it wrote a line it cannot honour. Withholding is never for an offer that is merely inconvenient. **The board obeys the same rule as the people**, or it is the loophole that hands out what conversation refuses. **Enforced, not merely stated**: every effect an NPC can offer must declare its availability rule in one table, and a test enumerates every topic on every NPC and fails on any effect with no entry — because **L49**'s whitelist was re-implemented verbatim from a paragraph that did not mention it had been amended, and a rule that lives only in prose gets un-followed by the next person to read the prose. When a model is put behind these NPCs (`brainstorm.md` §9.2), the same resolution runs on what it proposes: speech stays free, but the game never lets an NPC offer what the world has already answered |
| L93 | Pickpocketing, of NPCs and of players | **Players can steal from the person — from NPCs and from each other.** It belongs to the design it lands in rather than being bolted on: `moderation.md` §2 already rules that a player who robs your caravan **has played the game correctly**, and any policy treating that as abuse is a policy that deletes the game; **L73** made knowledge property, so there is already something to steal that is not an object; `economy.md` §6 gives Circles (**L24**) a black layer to belong to; and under **L18**'s classless skill-by-use there is no thief CLASS, only a set of hands that have practised. **The bound is already locked and is what makes this survivable**: `economy.md` §5 keeps wealth in things that sit somewhere guardable, with no remote access to stored goods — so what is on your person is what is at risk, never your net worth, and a lifted purse is a bad afternoon rather than a deleted character. **Being caught has to cost more than the purse**, or this collapses into free money at scale — the failure that has killed this feature in every game that shipped it without teeth. **Repeatedly targeting one player across sessions is where this stops being play**: `moderation.md`'s line is harm to a PERSON, not harm to a character, and following somebody to rob them nightly is stalking wearing a game mechanic. That line needs drawing before launch, not after the first report. **Open, and flagged rather than assumed**: what is liftable from a player (coin only, one carried item, knowledge); whether the victim knows, and when; what detection costs (a bounty, the law, the victim's right of reply); whether law-having settlements differ from the Interior; whether an offline or idle player can be a target at all; and how an attempt reads on screen with no HUD (**L80**) |
...
11. **Design completion pass** (2026-09-08) — **L40–L87 locked** and
    ...
    (L52–L53); the engine (L54); stamina as exertion, mobility, the
    ...
    (`sim/`) now implements and tests the rules — 267 tests, engine-free.
...
every document identified as missing has been written (88 decisions).
...
1. ~~**Hardware.** Stage 1 needs a machine that can run Unity.~~
   **Resolved by L54 (2026-09-14).** Godot 4.3 runs comfortably on the
   2017 MacBook Air, and Stage 1 steps 1–3 were in fact built with no
   developer machine involved at all. A Windows PC is still the better
