...
**Status:** 88 locked decisions, the original ten structural pillars
...
**The engine is Godot** — L54, revised 2026-09-14 from Unity, on the
evidence in [`design/tech.md`](design/tech.md) §2a. The deciding
argument was that the whole build-and-play loop runs on Godot with
nothing on the developer's machine; the two questions that were holding
the review (asset ecosystem, console cost) turned out not to favour
Unity either.
...
| [`prototype/`](prototype/) | **The game, such as it is** — the Godot project, Stage 1 steps 1–3, built headless with no editor. Started as the experiment that settled L54. |
...
| [`design/tech.md`](design/tech.md) | Engine (**L54 — Godot**), server architecture, data model, platform sequencing, the solo production strategy the whole plan rests on, the staged build order, and how the work actually gets divided. |
